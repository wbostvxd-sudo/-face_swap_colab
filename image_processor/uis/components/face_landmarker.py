...
from image_processor import face_landmarker, state_manager, translator
from image_processor.common_helper import calculate_float_step
from image_processor.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = image_processor.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(image_processor.choices.face_landmarker_score_range),
		minimum = image_processor.choices.face_landmarker_score_range[0],
		maximum = image_processor.choices.face_landmarker_score_range[-1]
	)
